...
async function loginChatEngine(username, password, email, fullName) {
  try {
    // Substitua as linhas a seguir pela lógica de autenticação do ChatEngine
    ...
    // Aqui, você pode lidar com a resposta do ChatEngine, se necessário
    console.log("Resposta do ChatEngine:", chatEngineResponse.data);

    // Exemplo: Se o ChatEngine retornar um token, você pode armazená-lo
    const chatEngineToken = chatEngineResponse.data.token;

    // Exemplo: Salvar o token em algum lugar seguro (local storage, etc.)
    localStorage.setItem("chatEngineToken", chatEngineToken);

  } catch (error) {
    // Lide com erros de autenticação do ChatEngine aqui
    console.error("Erro de autenticação no ChatEngine:", error);
    throw error; // Rejeitar a promessa para que o erro seja propagado para o chamador
  }
}
